...
func main() {
	...
	// ❌ WRONG: producer has no shutdown path; will block if consumer stops.
	newRandStream := func() <-chan int {
		randStream := make(chan int)
		go func() {
			defer fmt.Println("newRandStream closure exited.") // <- will NEVER print
			defer close(randStream)                             // <- will NEVER run
			for {
				randStream <- rand.Int() // blocks forever once consumer stops reading
			}
		}()
		return randStream
	}
	...
	fmt.Println("stopped reading; producer will now block on its next send...")

	// Give the producer a moment to hit the blocking send.
	time.Sleep(200 * time.Millisecond)

	// The producer goroutine is leaked (blocked on send).
	fmt.Println("goroutines after stopping consumption:", runtime.NumGoroutine())

	// Program exits; note the deferred "closure exited" never printed.
}
